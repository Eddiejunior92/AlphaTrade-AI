...
*   Day-trade tuning (May 2026): operating on `AGENT_INTERVAL_SECONDS=30`, `DAY_TRADING_DIP_REQUIREMENT_STRICTNESS=1` (Loose — any 1-of-5 dip conditions), `LLM_SKIP_PRICE_BPS=40`, `LLM_QUIET_SHORTCUT=false`, day strategy `maxHoldings=6`. Goal: ~10 day entries/day. Dip amplifier: when day-strategy BUY clears the dip gate AND `pctBelowVwap >= 0.5%` AND `cumDeltaSlope > 0` (mean-reversion setup with positive flow), `dipSizingMult=1.25` is passed via `dynamicWithUpgrades` to `riskManager.evaluateBuy` which clamps to `[1.00, 1.25]` defence-in-depth. Multiplier never exceeds 1.25, never fires for swing strategies, never bypasses the 4% `maxPositionPct` cap (qty is `min(qtyByRisk, qtyByPosition)`). All other safety rails (3-of-4 quorum, 80% confidence gate, daily loss budget, 5% drawdown circuit breaker, kill switch, no-averaging-in, recovery buffer, ASX min notional) untouched.
*   **Position cap raised to 4% (May 2026)**: Day strategies (`day` US, `asx_day` ASX) now use `maxPositionPct = 0.04` in `strategies.js`. Swing strategies (`swing`, `asx_swing`) keep their existing 0.05 cap. `max_position_pct` is now an allowlisted SAFE_KEY (Discord-approvable) with a HARD upper bound of **5%** enforced in `discordApprovalService.SAFE_KEYS.max_position_pct.validate` (range 0.01–0.05). Any future Discord-approved change writes to `portfolio.max_position_pct_day` AND emits a `POSITION_CAP_INCREASED` audit row. A one-shot `POSITION_CAP_INCREASED` migration audit row is written on first boot (`payload.migration_id = 'cap_3_to_4_may_2026'`). All other safety rails — kill switch, circuit breaker, daily-loss budget, drawdown breaker, recovery buffer, quorum, atomic cash, audit chain, confidence floor 65% — remain in DENIED_KEYS and CANNOT be changed via Discord under any circumstances.
