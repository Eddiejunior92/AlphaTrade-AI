...
## Dual-Broker Dashboard — Per-Market Toggles (May 2026)

Visually separates Alpaca (US, USD) from IBKR (ASX, AUD) with per-market master switches and per-market paper/live mode. ASX execution is intentionally **NOT wired** in this build — preference is persisted only. Every safety rail (3-of-4 quorum, 85% confidence, kill switch, breaker, daily loss budget, sizing, recovery buffer, day cadence, dip-buy filter) is **untouched**.

- **Schema (portfolio):** `us_market_enabled BOOLEAN DEFAULT TRUE`, `asx_market_enabled BOOLEAN DEFAULT TRUE`, `asx_trading_mode TEXT DEFAULT 'paper'`. Legacy `trading_mode` column remains the US source-of-truth (Alpaca client + dailyReset + compliance all read it). All three new fields added to `ALLOWED_PORTFOLIO_FIELDS`.
- **`marketRegistry.isAsxEnabled()` default flipped to ON.** Env stays a hard kill-switch (`ASX_ENABLED=false/0/no/off` disables).
- **Setters (`backend/agent.js`):** `setMarketEnabled(market, enabled)` audit-logs `MARKET_ENABLED_CHANGED`. `setMarketMode(market, mode)` waits for `cycleInProgress || flattenInProgress || tradingLock` (closes mode-switch race), updates `trading_mode` + `alpacaService.setMode` for US or `asx_trading_mode` for ASX, audit-logs `MARKET_MODE_CHANGED`. Legacy `setTradingMode(mode)` is now a shim → `setMarketMode('US', mode)`.
- **Endpoints (operator-gated):** `POST /api/agent/market-enabled` and `POST /api/agent/market-mode`. The market-mode endpoint stacks `requireOperator + requireOperatorStrict` so LIVE switches require `OPERATOR_TOKEN` configured AND provided AND `confirm: 'I_UNDERSTAND_LIVE'` (matches legacy `/trading-mode` policy).
- **CRITICAL — exits never gated:** Per-market master switches gate **NEW ENTRIES ONLY**. `runStrategy(sc, …, { noNewEntries })` always runs `evaluateExistingPositions` (stop-loss, take-profit, trailing) and `forceFlatten`-before-close — disabling a market never abandons open positions on the broker. The per-strategy `day_enabled / swing_enabled / asx_swing_enabled` toggles retain original full-skip behavior (operator intent to halt entirely).
- **CRITICAL — ASX execution hard-blocked:** `brokerRouter.placeOrder` throws `ASX_EXECUTION_DISABLED` for any ASX symbol unless `ASX_EXECUTION_WIRED=true` env is set. UI surfaces `executionWired:false` and the LIVE confirm modal explains "preference saved — won't place real orders until IBKR is wired". To enable real IBKR routing in the future: set `ASX_EXECUTION_WIRED=true` AND provide `IBKR_BASE_URL` + `IBKR_ACCOUNT_ID`.
- **State exposure (`/api/state`):** `markets: { US: {enabled, mode, broker:'Alpaca', currency:'USD', flag, executionWired:true, liveAvailable, open, nextOpen, nextClose}, ASX: {…broker:'IBKR', executionWired:false, envDisabled} }` and `equityBreakdown: { usHoldingsUSD, usCashUSD, usTotalUSD, asxHoldingsAUD, asxHoldingsUSD, audUsdRate, totalUSD }`. FX rate read from `fxService.getStatus().audusd` (no `.rates` wrapper — earlier draft used a non-existent key and silently dropped ASX USD value).
- **Frontend (`frontend/src/App.jsx`):** Header replaces single PAPER/LIVE badge with two `MarketModeBadge` (flag + mode + `·preview` suffix when `executionWired:false`). Hero card adds dual equity-breakdown tiles (US: `$X` + cash; ASX: `A$Y ≈ $Z USD @ rate`). New `MarketsAndModesPanel` on Home with per-market on/off toggle + paper/live buttons. Holdings + Trades tabs split into per-market sections with `MarketSectionHeader` (broker · currency · mode · open/closed). The mode-switch confirmation modal is now per-market (`{market, target}`) — ASX-LIVE shows a yellow "preference saved" note instead of the red Alpaca live-money warning.
- **Backwards compat:** Legacy `POST /api/agent/trading-mode` still works unchanged. Existing audit/compliance/dailyReset readers of `trading_mode` keep working — `setMarketMode('US', …)` writes both the legacy column and `alpacaService.setMode`.
